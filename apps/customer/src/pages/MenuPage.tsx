...
import { getPublicMenu, placeOrder, placeTakeawayOrder, validateCoupon, getRecommendations, getCustomerHistory, getAvailableCoupons } from '../lib/api';
...
export default function MenuPage() {
  ...
  const isTakeawayMode = (searchParams.get('mode') === 'takeaway' || useCartStore((state) => state.takeawayMode)) && !tableId;

  // Auto-redirect spice-garden demo to table 1 if no tableId provided and not in takeaway mode
  useEffect(() => {
    if (slug === 'spice-garden' && !tableId && searchParams.get('mode') !== 'takeaway') {
      setSearchParams({ table: '1' });
    }
  }, [slug, tableId, searchParams, setSearchParams]);
  ...
  const [orderType, setOrderType] = useState<'DINE_IN' | 'TAKE_AWAY'>(isTakeawayMode ? 'TAKE_AWAY' : 'DINE_IN');
  ...
  const { data: menuData, isLoading, error } = useQuery<MenuData>({
    queryKey: ['menu', slug, tableId, isTakeawayMode],
    queryFn: () => getPublicMenu(slug, isTakeawayMode ? 'takeaway' : tableId) as Promise<MenuData>,
    enabled: !!slug && (!!tableId || isTakeawayMode),
  });
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (isTakeawayMode) {
        if (!menuData?.branch) throw new Error("Branch not found");
        result = await placeTakeawayOrder({
          branchId: menuData.branch.id || (menuData as any).restaurant?.branchId || '', // Assuming branch info is available
          items: payloadItems,
              ...
              <div className="px-4 py-4 border-b border-white/5">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setVegOnly(!vegOnly); if (!vegOnly) setNonVegOnly(false); }}
                    className={`flex-1 flex items-center justify-center gap-2.5 px-3 py-2.5 rounded-xl border text-[11px] font-bold transition-all ${vegOnly ? 'bg-green-500/10 text-green-400 border-green-500/30 shadow-[0_0_15px_rgba(34,197,94,0.1)]' : 'border-white/10 text-white/60 hover:border-white/20'
                      }`}>
                  ...
                  <button onClick={() => { setNonVegOnly(!nonVegOnly); if (!nonVegOnly) setVegOnly(false); }}
                    className={`flex-1 flex items-center justify-center gap-2.5 px-3 py-2.5 rounded-xl border text-[11px] font-bold transition-all ${nonVegOnly ? 'bg-red-500/10 text-red-400 border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 'border-white/10 text-white/60 hover:border-white/20'
                      }`}>
              ...
              <div className="px-4 py-4 flex-1">
                ...
                <div className="flex flex-col gap-1">
                  <button onClick={() => setActiveCategory('all')}
                    className={`flex items-center justify-between w-full px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${activeCategory === 'all' ? 'bg-[#D97706] text-black' : 'text-white/60 hover:bg-white/5'
                      }`}>
                    <span>All Items</span>
                  ...
                  {menuData.categories.map((cat) => (
                    <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
                      className={`flex items-center justify-between w-full px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${activeCategory === cat.id ? 'bg-[#D97706] text-black' : 'text-white/60 hover:bg-white/5'
                        }`}>
                      <span>{cat.name}</span>
          ...
          <div className="lg:hidden pb-[120px]">
            ...
            <div className="px-6 flex items-center gap-3 overflow-x-auto no-scrollbar py-4 sticky top-20 z-30 bg-[#0A0A0A]/90 backdrop-blur-md border-b border-white/5">
              ...
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl border text-[10px] font-black tracking-widest transition-all uppercase ${vegOnly ? 'bg-green-500/10 text-green-500 border-green-500/30' : 'bg-white/5 border-white/10 text-white/40 hover:border-white/20'
                  }`}
              >
              ...
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl border text-[10px] font-black tracking-widest transition-all uppercase ${nonVegOnly ? 'bg-red-500/10 text-red-500 border-red-500/30' : 'bg-white/5 border-white/10 text-white/40 hover:border-white/20'
                  }`}
              >
              ...
              <button
                ...
                className={`flex-shrink-0 px-6 py-2 rounded-xl text-[10px] font-black tracking-widest transition-all uppercase ${activeCategory === 'all' ? 'bg-brand-primary text-white shadow-[0_8px_20px_rgba(217,119,6,0.2)]' : 'bg-white/5 border border-white/10 text-white/40 hover:bg-white/10'
                  }`}
              >
                All
              </button>
              {menuData.categories.map((cat) => (
                <button
                  ...
                  className={`flex-shrink-0 px-6 py-2 rounded-xl text-[10px] font-black tracking-widest transition-all whitespace-nowrap uppercase ${activeCategory === cat.id ? 'bg-brand-primary text-white shadow-[0_8px_20px_rgba(217,119,6,0.2)]' : 'bg-white/5 border border-white/10 text-white/40 hover:bg-white/10'
                    }`}
                >
                  {cat.name}
                </button>
                  ...
                  {history.map((order: any) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        navigate(`/track/${order.sessionId}`);
                      }}
                      className="block w-full text-left cursor-pointer group bg-white/[0.03] rounded-[2rem] p-6 border border-white/5 hover:border-[#D97706]/50 hover:bg-white/[0.05] transition-all duration-300"
                    >
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[0.1em] border ${order.status === 'COMPLETED' ? 'bg-green-500/10 border-green-500/20 text-green-400' :
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 sm:p-0">
          ...
          <div className="relative w-full max-w-lg bg-[#0A0A0A] rounded-[3rem] p-8 sm:p-10 animate-slide-up border border-white/10 shadow-[0_32px_100px_rgba(0,0,0,1)] text-white overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
              <div>
                <p className="text-[10px] font-black text-[#D97706] uppercase tracking-[0.4em] mb-2">Order Summary</p>
            ...
            <div className="space-y-6 mb-8 relative z-10 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
              <div className={`${isTakeawayMode ? 'flex' : 'grid grid-cols-2'} gap-4 mb-6`}>
                {!isTakeawayMode && (
                  <button
                    type="button"
                    onClick={() => setOrderType('DINE_IN')}
                    className={`py-4 rounded-2xl flex flex-col items-center justify-center gap-2 border transition-all ${orderType === 'DINE_IN'
                ...
                <button
                  ...
                  className={`py-4 rounded-2xl flex flex-col items-center justify-center gap-2 border transition-all ${isTakeawayMode ? 'w-full' : ''} ${orderType === 'TAKE_AWAY'
                      ? 'bg-saffron-500/10 border-saffron-500 text-saffron-500 shadow-[0_0_20px_rgba(245,158,11,0.1)]'
                      : 'bg-white/5 border-white/10 text-white/40 hover:bg-white/10'
                    }`}
                >
                  <ShoppingCart size={20} />
                  <span className="text-[10px] font-black uppercase tracking-widest">Parcel</span>
                </button>
              </div>
            ...
            <p className="text-center text-[9px] text-white/20 font-bold uppercase tracking-[0.3em] mt-6 leading-relaxed">
              Your order will be prepared fresh <br /> upon confirmation.
            </p>
          </div>
        </div>,
        document.body
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {safeVariants.map((v) => (
                      <label
                        key={v.id}
                        className={`group flex items-center justify-between p-6 rounded-[2rem] cursor-pointer transition-all border-2 ${selectedVariant === v.id ? 'bg-[#D97706]/5 border-[#D97706] shadow-[0_10px_30px_rgba(217,119,6,0.1)]' : 'bg-white/[0.02] border-white/5 hover:bg-white/[0.05]'
                          }`}
                      >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {safeAddons.map((a) => (
                      <label
                        key={a.id}
                        className={`group flex items-center justify-between p-6 rounded-[2rem] cursor-pointer transition-all border-2 ${selectedAddons.includes(a.id) ? 'bg-[#D97706]/5 border-[#D97706] shadow-[0_10px_30px_rgba(217,119,6,0.1)]' : 'bg-white/[0.02] border-white/5 hover:bg-white/[0.05]'
                          }`}
                      >
                        <div className="flex items-center gap-5">
