...
router.get('/orders', asyncHandler(async (req, res) => {
    ...
    const from = query.from ? new Date(query.from) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const to = query.to ? new Date(query.to) : new Date();
    const where = { restaurantId, createdAt: { gte: from, lte: to } };
    if (query.branchId)
        where['branchId'] = query.branchId;
...
// Menu performance
router.get('/menu-performance', asyncHandler(async (req, res) => {
    const restaurantId = req.user.restaurantId;
...
// Table performance
router.get('/table-performance', asyncHandler(async (req, res) => {
    const restaurantId = req.user.restaurantId;
    ...
    const data = tables.map((table) => {
        ...
        const sessionDurations = table.orders.map((o) => (o.updatedAt.getTime() - o.createdAt.getTime()) / 60000);
