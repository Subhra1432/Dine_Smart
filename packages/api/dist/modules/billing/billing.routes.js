...
// Update order status
router.put('/orders/:orderId/status', requireRole(['MANAGER', 'CASHIER']), asyncHandler(async (req, res) => {
    const { status } = updateOrderStatusSchema.parse(req.body);
    const data = await ordersService.updateOrderStatus(req.params['orderId'], req.user.restaurantId, status);
    res.json({ success: true, data });
}));
// Update payment
router.put('/orders/:orderId/payment', requireRole(['MANAGER', 'CASHIER']), asyncHandler(async (req, res) => {
...
// Print bill
router.post('/orders/:orderId/print-bill', asyncHandler(async (req, res) => {
    const order = await ordersService.getOrder(req.params['orderId'], req.user.restaurantId);
...
<html><head><style>
  ...
  <div class="center">
    ...
    <p>Table #${order.table ? order.table['number'] : ''}</p>
...
// Refund
router.post('/orders/:orderId/refund', requireRole(['MANAGER', 'CASHIER']), asyncHandler(async (req, res) => {
...
router.put('/orders/:orderId/items/:itemId', requireRole(['MANAGER', 'CASHIER']), asyncHandler(async (req, res) => {
    ...
    if (item.status !== 'PENDING')
        throw new AppError(400, 'Can only edit items that are not yet being prepared');
