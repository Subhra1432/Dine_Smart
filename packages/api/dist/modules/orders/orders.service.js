...
export async function createOrder(data) {
    ...
    const orderItemsData = data.items.map((item) => {
        ...
        if (item.variantId) {
            ...
            if (!variant)
                throw new AppError(400, `Variant ${item.variantId} not found`);
        ...
        if (item.addonIds && item.addonIds.length > 0) {
            for (const addonId of item.addonIds) {
                ...
                if (!addon)
                    throw new AppError(400, `Addon ${addonId} not available for this item`);
...
export async function getOrder(orderId, restaurantId) {
...
export async function updatePayment(orderId, restaurantId, paymentStatus, paymentMethod) {
    const order = await prisma.order.findFirst({ where: { id: orderId, restaurantId } });
    if (!order)
        throw new AppError(404, 'Order not found');
...
export async function requestPaymentAttention(orderId) {
    const order = await prisma.order.findUnique({
        where: { id: orderId },
        include: { table: true },
    });
