...
router.post('/restaurants', asyncHandler(async (req, res) => {
    ...
    if (existing)
        throw new AppError(400, 'Restaurant slug already exists');
...
router.put('/restaurants/:id/plan', asyncHandler(async (req, res) => {
...
router.put('/restaurants/:id/suspend', asyncHandler(async (req, res) => {
    const restaurant = await prisma.restaurant.findUnique({ where: { id: req.params['id'] } });
    if (!restaurant)
        throw new AppError(404, 'Restaurant not found');
    const updated = await prisma.restaurant.update({
...
router.get('/stats', asyncHandler(async (req, res) => {
    ...
    const planPrices = { STARTER: 999, GROWTH: 2499, PREMIUM: 7499 };
...
router.post('/restaurants/:id/impersonate', asyncHandler(async (req, res) => {
    const restaurant = await prisma.restaurant.findUnique({ where: { id: req.params['id'] } });
    if (!restaurant)
        throw new AppError(404, 'Restaurant not found');
