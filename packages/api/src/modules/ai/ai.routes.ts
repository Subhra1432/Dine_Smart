...
router.get('/recommendations', publicRateLimiter, asyncHandler(async (req: Request, res: Response) => {
  const itemIds = (req.query['itemIds'] as string || '').split(',').filter(Boolean);
...
router.get('/demand-forecast', authenticate, requireRole(['OWNER', 'MANAGER']), requirePlanFeature('aiDemandForecast'),
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const itemHourCounts = new Map<string, Map<number, number[]>>();
    for (const oi of orderItems) {
      const itemId = oi.menuItemId;
      const hour = oi.order.createdAt.getHours();

      if (!itemHourCounts.has(itemId)) itemHourCounts.set(itemId, new Map());
...
router.get('/pricing-suggestions', authenticate, requireRole(['OWNER', 'MANAGER']), requirePlanFeature('aiSmartPricing'),
  asyncHandler(async (req: Request, res: Response) => {
    const restaurantId = req.user!.restaurantId;
