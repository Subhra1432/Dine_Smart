...
import { createOrderSchema, initiatePaymentSchema, razorpayWebhookSchema, createReviewSchema } from '@dinesmart/shared';
...
export async function updatePublicOrderItem(req: Request, res: Response) {
  const orderId = req.params['orderId']!;
  const itemId = req.params['itemId']!;
  const data = req.body;
  const result = await ordersService.updatePublicOrderItem(orderId, itemId, data);
